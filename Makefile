...
BENTOML_MODEL= $(shell bentoml models list -o json | grep -oP '(?<="tag": ")\w+:\w+' | grep $(PROJECT) | head -n1 | sed "s/:/\\\:/g")
BENTOML_CLASSIFIER= $(shell bentoml list -o json | grep -oP '(?<="tag": ")\w+:\w+' | grep $(PROJECT) | head -n1 | sed "s/:/\\\:/g")
